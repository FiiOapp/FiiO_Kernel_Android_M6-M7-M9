...
/**
 * Generate interrupt
 * @param[in]	pdev	pointer to struct platform_device of target abox_gic device
 * @param[in]	hw_irq	hardware irq number
 */
extern void abox_gic_generate_interrupt(struct platform_device *pdev, int hw_irq);
 ...
 */
extern void abox_gic_register_irq_handler(struct platform_device *pdev, irq_handler_t irq_handler, void *dev_id);
...
/**
 * Enable ABOX GIC irq
 * @param[in]	dev	pointer to struct device of target abox_gic device
 */
...
extern void abox_gicc_enable(struct platform_device *pdev, int en);
