...
static irqreturn_t abox_wdma_irq_handler(int irq, void *dev_id, ABOX_IPC_MSG *msg)
{
	...
	struct snd_pcm_hw_params *params)
{
	...
	if (abox_is_bt_probed() && data->abox_data->bt_status &&
		id == ABOX_ID_WDMA3)
		pcmtask_msg->param.setbuff.phyaddr = REALTIME_BASE;
	else
	...
	if (data->type == PLATFORM_REALTIME) {
		msg.ipcid = IPC_ERAP;
...
static int abox_wdma_hw_free(struct snd_pcm_substream *substream)
{
	...
	exynos_sysmmu_tlb_invalidate(data->abox_data->iommu_domain, (dma_addr_t)IOVA_WDMA_BUFFER(id),
			round_up(substream->runtime->dma_bytes, PAGE_SIZE));
#endif
...
static int abox_wdma_prepare(struct snd_pcm_substream *substream)
{
	...
	msg.task_id = pcmtask_msg->channel_id = id;
	result = abox_start_ipc_transaction(&data->pdev_abox->dev, msg.ipcid, &msg, sizeof(msg), 0, 1);
...
static int abox_wdma_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		result = abox_start_ipc_transaction(&data->pdev_abox->dev, msg.ipcid, &msg, sizeof(msg), 1, 0);
		switch (data->type) {
...
static int abox_wdma_close(struct snd_pcm_substream *substream)
{
	...
	case PLATFORM_CALL:
		break;
	default:
		for (i = ABOX_DMA_TIMEOUT_US; (readl(data->sfr_base + ABOX_WDMA_CTRL)
...
static int abox_wdma_new(struct snd_soc_pcm_runtime *runtime)
{
	...
	if (IS_ERR_VALUE(result)) {
		...
		iommu_map(data->abox_data->iommu_domain, IOVA_WDMA_BUFFER(id), substream->dma_buffer.addr, BUFFER_BYTES_MAX, 0);
#endif
	}
...
static void abox_wdma_free(struct snd_pcm *pcm)
{
	...
	iommu_unmap(data->abox_data->iommu_domain, IOVA_WDMA_BUFFER(id), BUFFER_BYTES_MAX);
#endif
