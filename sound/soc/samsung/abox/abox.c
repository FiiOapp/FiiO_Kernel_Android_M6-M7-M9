		...
		struct snd_pcm_hw_params *params)
{
	...
	if (params_channels(params) < 1) {
		dev_dbg(dev, "channel count is fixed from %d to 2\n", params_channels(params));
		hw_param_interval(params, SNDRV_PCM_HW_PARAM_CHANNELS)->min = 2;
	}

	if (params_width(params) < 16) {
		dev_dbg(dev, "bit depth is fixed from %d to 16\n", params_width(params));
	...
	if (playback) {
		...
		case 0x1:
			...
			dev_dbg(dev, "sampling rate is fixed to %u\n", hw_param_interval(params, SNDRV_PCM_HW_PARAM_RATE)->min);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = data->bt_status;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	data->bt_status = (unsigned int)ucontrol->value.integer.value[0];

	if (data->bt_probed) {
		dev_info(dev, "%s bt sco status(%d)\n",
						__func__, data->bt_status);
		msg.ipcid = IPC_SYSTEM;
		system_msg->msgtype = ABOX_BT_SCO_ENABLE;
		system_msg->param1 = data->bt_status;
		system_msg->param2 = BT_SHARED_MEMORY;

		abox_start_ipc_transaction(dev, msg.ipcid, &msg, sizeof(msg), 0, 1);
	} else {
		data->bt_status = false;
		dev_err(dev, "%s bt sco device is not connected\n", __func__);
	}
...
static const char *abox_sound_type_enum_texts[] = {
	"VOICE",
	"SPEAKER",
	"HEADSET",
	"BTVOICE",
...
SOC_VALUE_ENUM_SINGLE_DECL(abox_sound_type_enum, SND_SOC_NOPM, 0, 0, abox_sound_type_enum_texts, abox_sound_type_enum_values);
...
static const struct snd_kcontrol_new abox_codec_controls[] = {
	...
	SOC_SINGLE_EXT("BT SCO Enable", SND_SOC_NOPM, 0, 1, 0, abox_bt_sco_get, abox_bt_sco_put),
	SOC_VALUE_ENUM_EXT("Sound Type", abox_sound_type_enum, abox_sound_type_get, abox_sound_type_put),
};
...
static void abox_change_cpu_gear(struct device *dev, struct abox_data *data)
{
	...
	if (!IS_ENABLED(CONFIG_SOC_EXYNOS7872) && (gear >= CPU_GEAR_LOWER_LIMIT)) {
...
static int abox_enable(struct device *dev)
{
	...
	abox_gicc_enable(data->pdev_gic, true);
...
static int samsung_abox_probe(struct platform_device *pdev)
{
	...
	iommu_map(data->iommu_domain, 0x10590000, 0x10590000, 0x10000, 0);
	...
	data->idle_ip_index = exynos_get_idle_ip_index(dev_name(&pdev->dev));
	if (data->idle_ip_index < 0)
		dev_err(dev, "Idle ip index is not provided for ABOX.\n");
...
static void samsung_abox_shutdown(struct platform_device *pdev)
{
	...
	if (data && data->regmap) {
		for (i = ABOX_UAIF0; i <= ABOX_UAIF3; i++)
			regmap_update_bits(data->regmap, ABOX_UAIF_CTRL0(i),
				ABOX_SPK_ENABLE_MASK, 0 << ABOX_SPK_ENABLE_L);
	}
	pm_runtime_disable(dev);
}
...
static void samsung_abox_bt_probe(struct scsc_bt_audio_driver *driver, struct scsc_bt_audio *bt_audio)
{
	...
	if (data->calliope_state == CALLIOPE_ENABLED) {
		...
		pr_info("%s trying to map size 0x%lx\n", __func__,
				ALIGN(sizeof(struct scsc_bt_audio_abox), 0x1000));
...
static void samsung_abox_bt_remove(struct scsc_bt_audio *bt_audio)
{
	...
	if (data->bt_probed == true) {
		...
		abox_start_ipc_transaction(dev, msg.ipcid, &msg, sizeof(msg), 0, 1);
		iommu_unmap(data->iommu_domain, BT_SHARED_MEMORY, ALIGN(sizeof(struct scsc_bt_audio_abox), 0x1000));
		exynos_sysmmu_tlb_invalidate(data->iommu_domain, (dma_addr_t)BT_SHARED_MEMORY,
			ALIGN(sizeof(struct scsc_bt_audio_abox), 0x1000));
		data->bt_probed = false;
	}
}
