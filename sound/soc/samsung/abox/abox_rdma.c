...
const static struct snd_compr_caps abox_rdma_compr_caps = {
	...
	.max_fragments		= 5,
	.num_codecs		= 3,
	.codecs			= {SND_AUDIOCODEC_MP3, SND_AUDIOCODEC_AAC, SND_AUDIOCODEC_FLAC},
};
...
static int abox_rdma_start_ipc_transaction(struct device *dev,
		int hw_irq, const void *supplement,
		size_t size, int atomic, int sync)
{
	struct abox_platform_data *platform_data = dev_get_drvdata(dev);
	return abox_start_ipc_transaction(&platform_data->pdev_abox->dev,
...
static int abox_rdma_mailbox_send_cmd(struct device *dev, unsigned int cmd)
{
	...
	case CMD_COMPR_SET_PARAM:
		...
		abox_rdma_mailbox_write(dev, abox_data, COMPR_PARAM_RATE, data->out_sample_rate);
#endif
		break;
	...
	result = abox_rdma_start_ipc_transaction(dev, IPC_OFFLOAD, NULL, 0, 1, 0);

	for (n = 0, ack = 0; n < 2000; n++) {
		if (abox_rdma_mailbox_read(dev, COMPR_ACK)) {	/* Wait for ACK */
			ack = 1;
			break;
		}
		udelay(100);
	}
	abox_rdma_mailbox_write(dev, COMPR_ACK, 0);		/* clear ACK */
...
static int abox_rdma_compr_isr_handler(void *priv)
{
	...
	case INTR_DECODED:
		...
		if (val) {
			...
			u32 size = abox_rdma_mailbox_read(dev, COMPR_SIZE_OUT_DATA);
			struct snd_compr_runtime *runtime = data->cstream->runtime;
			...
			if (!data->start &&
				...
				u64 bytes_available = data->received_total - data->copied_total;
...
static int abox_rdma_compr_set_param(struct platform_device *pdev, struct snd_compr_runtime *runtime)
{
	struct device *dev = &pdev->dev;
	...
	dev_info(dev, "%s[%d] buffer: %p(%llu)\n", __func__, id, runtime->buffer, runtime->buffer_size);
	...
	runtime->buffer = dma_alloc_coherent(dev, runtime->buffer_size, &data->dma_addr, GFP_KERNEL);
	if (!runtime->buffer) {
...
#else
	data->dma_addr = dma_map_single(dev, runtime->buffer, runtime->buffer_size, DMA_TO_DEVICE);
...
#endif
	ret = iommu_map(platform_data->abox_data->iommu_domain, IOVA_COMPR_BUFFER(id),
			virt_to_phys(runtime->buffer), round_up(runtime->buffer_size, PAGE_SIZE), 0);
	if (IS_ERR_VALUE(ret)) {
	...
	abox_rdma_mailbox_write(dev, COMPR_PHY_ADDR_INBUF, IOVA_COMPR_BUFFER(id));
...
#else
{
	...
	iommu_unmap(platform_data->abox_data->iommu_domain, IOVA_COMPR_BUFFER(id),
			round_up(runtime->buffer_size, PAGE_SIZE));
	exynos_sysmmu_tlb_invalidate(platform_data->abox_data->iommu_domain, (dma_addr_t)IOVA_COMPR_BUFFER(id),
			round_up(runtime->buffer_size, PAGE_SIZE));

#ifdef COMPR_USE_COPY
	dma_free_coherent(dev, runtime->buffer_size, runtime->buffer, data->dma_addr);
	runtime->buffer = NULL;
#else
	dma_unmap_single(dev, data->dma_addr, runtime->buffer_size, DMA_TO_DEVICE);
#endif
}
#endif
			      ...
			      struct snd_compr_metadata *metadata)
{
	...
	if (metadata->key == SNDRV_COMPRESS_ENCODER_PADDING) {
		dev_dbg(dev, "%s: got encoder padding %u", __func__, metadata->value[0]);
	} else if (metadata->key == SNDRV_COMPRESS_ENCODER_DELAY) {
		dev_dbg(dev, "%s: got encoder delay %u", __func__, metadata->value[0]);
	}
...
static int abox_rdma_compr_trigger(struct snd_compr_stream *stream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		dev_info(dev, "%s: %s", __func__, (cmd == SNDRV_PCM_TRIGGER_START) ?
	...
	case SND_COMPR_TRIGGER_DRAIN:
		dev_info(dev, "%s: %s", __func__, (cmd == SND_COMPR_TRIGGER_DRAIN) ?
			 ...
			 struct snd_compr_tstamp *tstamp)
{
	...
	if (pcm_size) {
		...
		dev_dbg(dev, "%s: pcm_size(%u), frame_count(%u), copied_total(%u)\n",
				__func__, pcm_size, tstamp->pcm_io_frames, tstamp->copied_total);
...
static int abox_rdma_compr_copy(struct snd_compr_stream *stream, char __user *buf, size_t count)
{
	struct snd_soc_pcm_runtime *rtd = stream->private_data;
	struct snd_soc_platform *platform = rtd->platform;
	struct device *dev = platform->dev;
		...
		struct snd_pcm_hw_params *params)
{
	...
	case 1:
		/* 192kHz 24bit */
		hw_param_interval(params, SNDRV_PCM_HW_PARAM_RATE)->min = 192000;
		snd_mask_reset(hw_param_mask(params, SNDRV_PCM_HW_PARAM_FORMAT),
				SNDRV_PCM_FORMAT_S16);
		snd_mask_set(hw_param_mask(params, SNDRV_PCM_HW_PARAM_FORMAT),
				SNDRV_PCM_FORMAT_S24);
	...
	case 2:
		/* 384kHz 32bit */
		hw_param_interval(params, SNDRV_PCM_HW_PARAM_RATE)->min = 384000;
		snd_mask_reset(hw_param_mask(params, SNDRV_PCM_HW_PARAM_FORMAT),
				SNDRV_PCM_FORMAT_S16);
		snd_mask_reset(hw_param_mask(params, SNDRV_PCM_HW_PARAM_FORMAT),
				SNDRV_PCM_FORMAT_S24);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	struct device *dev = platform->dev;
	struct abox_platform_data *platform_data = snd_soc_platform_get_drvdata(platform);
...
static const DECLARE_TLV_DB_LINEAR(abox_rdma_compr_vol_gain, 0, COMPRESSED_LR_VOL_MAX_STEPS);
...
static const char *abox_rdma_compr_format_text[] = {
	"48kHz 16bit",
	"192kHz 24bit",
	"384kHz 32bit",
};
...
static const struct snd_kcontrol_new abox_rdma_compr_controls[] = {
	SOC_DOUBLE_R_EXT_TLV("ComprTx0 Volume", COMPR_LEFT_VOL, COMPR_RIGHT_VOL, 0, COMPRESSED_LR_VOL_MAX_STEPS, 0,
			abox_rdma_compr_vol_get, abox_rdma_compr_vol_put, abox_rdma_compr_vol_gain),
...
static irqreturn_t abox_rdma_irq_handler(int irq, void *dev_id, ABOX_IPC_MSG *msg)
{
	...
	dev_dbg(dev, "%s[%d]: ipcid=%d, msgtype=%d\n", __func__, id, msg->ipcid, pcmtask_msg->msgtype);
	...
	struct snd_pcm_hw_params *params)
{
	...
	result = snd_pcm_lib_malloc_pages(substream, params_buffer_bytes(params));
	...
	result = iommu_map(data->abox_data->iommu_domain, IOVA_RDMA_BUFFER(id),
			runtime->dma_addr, round_up(runtime->dma_bytes, PAGE_SIZE), 0);
	if (IS_ERR_VALUE(result)) {
	...
	if (abox_is_bt_probed() && data->abox_data->bt_status &&
		id == ABOX_ID_RDMA6) {
	...
	abox_rdma_start_ipc_transaction(dev, msg.ipcid, &msg, sizeof(msg), 0, 1);

	pcmtask_msg->msgtype = PCM_PLTDAI_HW_PARAMS;
	pcmtask_msg->param.hw_params.sample_rate = params_rate(params);
	pcmtask_msg->param.hw_params.bit_depth = snd_pcm_format_width(params_format(params));
...
static int abox_rdma_hw_free(struct snd_pcm_substream *substream)
{
	...
	exynos_sysmmu_tlb_invalidate(data->abox_data->iommu_domain, (dma_addr_t)IOVA_RDMA_BUFFER(id),
			round_up(substream->runtime->dma_bytes, PAGE_SIZE));
#endif
...
static int abox_rdma_prepare(struct snd_pcm_substream *substream)
{
	...
	msg.task_id = pcmtask_msg->channel_id = id;
	result = abox_rdma_start_ipc_transaction(dev, msg.ipcid, &msg, sizeof(msg), 0, 1);
...
static int abox_rdma_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (PHYS_OFFSET < substream->runtime->dma_addr &&
				substream->runtime->dma_addr < MAX_MEMBLOCK_ADDR) {
			abox_request_dram_on(data->pdev_abox, dev, true);
		}

		pcmtask_msg->param.trigger = 1;
		result = abox_rdma_start_ipc_transaction(dev, msg.ipcid, &msg, sizeof(msg), 1, 0);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
...
static int abox_rdma_open(struct snd_pcm_substream *substream)
{
	...
	pm_runtime_get_sync(rtd->codec->dev);
	mutex_lock(&data->abox_data->sicd_mutex);
	atomic_inc(&data->abox_data->use_cnt);
	if (atomic_read(&data->abox_data->use_cnt) >= 1) {
		exynos_update_ip_idle_status(data->abox_data->idle_ip_index, 0);
	}
	mutex_unlock(&data->abox_data->sicd_mutex);
...
static int abox_rdma_close(struct snd_pcm_substream *substream)
{
	...
	switch (data->type) {
	default:
		for (i = ABOX_DMA_TIMEOUT_US; (readl(data->sfr_base + ABOX_RDMA_CTRL0)
	...
	pm_runtime_put(rtd->codec->dev);
	mutex_lock(&data->abox_data->sicd_mutex);
	atomic_dec(&data->abox_data->use_cnt);
	if (atomic_read(&data->abox_data->use_cnt) < 1) {
		exynos_update_ip_idle_status(data->abox_data->idle_ip_index, 1);
	}
	mutex_unlock(&data->abox_data->sicd_mutex);
	if (data->type == PLATFORM_CALL) {
		abox_request_cpu_gear_sync(dev, data->abox_data,
...
{
	struct snd_pcm_substream *substream = runtime->pcm->streams[SNDRV_PCM_STREAM_PLAYBACK].substream;
	...
	result = snd_pcm_lib_preallocate_pages(substream, SNDRV_DMA_TYPE_DEV, runtime->cpu_dai->dev, buffer_bytes, buffer_bytes);
	if (IS_ERR_VALUE(result)) {
		result = -ENOMEM;
	} else {
		struct snd_soc_platform *platform = runtime->platform;
		...
		iommu_map(data->abox_data->iommu_domain, IOVA_RDMA_BUFFER(id), substream->dma_buffer.addr, BUFFER_BYTES_MAX, 0);
#endif
	}
...
static void abox_rdma_free(struct snd_pcm *pcm)
{
	...
	iommu_unmap(data->abox_data->iommu_domain, IOVA_RDMA_BUFFER(id), BUFFER_BYTES_MAX);
#endif
...
{
	struct device *dev = platform->dev;
	struct abox_platform_data *data = snd_soc_platform_get_drvdata(platform);
	int result;
	...
	if (data->type == PLATFORM_COMPRESS) {
		...
		result = snd_soc_add_platform_controls(platform,
				abox_rdma_compr_controls, ARRAY_SIZE(abox_rdma_compr_controls));
		if (IS_ERR_VALUE(result)) {
			dev_err(dev, "adding platform control failed: %d\n", result);
			return result;
		}
#ifdef COMPR_USE_FIXED_MEMORY
		compr_data->dma_size = abox_rdma_compr_caps.max_fragments * abox_rdma_compr_caps.max_fragment_size;
		compr_data->dma_area = dmam_alloc_coherent(dev, compr_data->dma_size, &compr_data->dma_addr, GFP_KERNEL);
		if (compr_data->dma_area == NULL) {
			dev_err(dev, "dma memory for compress offload allocation failed: %lu\n", PTR_ERR(compr_data->dma_area));
			return -ENOMEM;
		}
		result = iommu_map(data->abox_data->iommu_domain, IOVA_COMPR_BUFFER(data->id),
				compr_data->dma_addr, round_up(compr_data->dma_size, PAGE_SIZE), 0);
		if (IS_ERR_VALUE(result)) {
			dev_err(dev, "dma memory for compress offload iommu map failed: %d\n", result);
			return result;
		}
#endif
	}
...
static int abox_rdma_remove(struct snd_soc_platform *platform)
{
	...
	if (data->type == PLATFORM_COMPRESS) {
		...
		iommu_unmap(data->abox_data->iommu_domain, IOVA_RDMA_BUFFER(data->id),
