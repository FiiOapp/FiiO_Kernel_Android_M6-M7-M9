...
{
	switch (reg) {
	case COD3034X_01_IRQ1PEND ... COD3034X_0D_STATUS5:
	case COD3034X_10_PD_REF ... COD3034X_1D_SV_DA:
	case COD3034X_20_VOL_AD1 ... COD3034X_27_MIX_AD2:
	case COD3034X_32_VOL_EP_SPK ... COD3034X_38_MIX_DA2:
	...
	case COD3034X_80_PDB_ACC1 ... COD3034X_8B_CTR_BDNC3:
	case COD3034X_90_CTR_DLY5 ... COD3034X_97_TEST3:
...
static bool cod3034x_writeable_register(struct device *dev, unsigned int reg)
{
	...
	case COD3034X_70_CLK1_COD ... COD3034X_7B_MAN_GN2:
	case COD3034X_80_PDB_ACC1 ... COD3034X_8F_CTR_DLY4:
	case COD3034X_90_CTR_DLY5 ... COD3034X_97_TEST3:
	case COD3034X_D0_CTRL_IREF1 ... COD3034X_DE_CTRL_SPKS2:
	case 0xF0 ... 0xFF:
...
/**
 * cod3034x_avc_ctvol_hp_tlv
 *
 * Range: 0dB to +4dB, step 1dB
 *
 * CTVOL_HP, reg(0x57), shift(0), width(3), invert(1), max(5)
 */
static const DECLARE_TLV_DB_SCALE(cod3034x_avc_ctvol_hp_tlv, 0, 100, 0);
 ...
 */
static const DECLARE_TLV_DB_SCALE(cod3034x_ctvol_ep_tlv, 0, 100, 0);
 ...
 *
 * CTVOL_SPK_PGA, reg(0x32), shift(0), width(4), invert(1), max(9)
 */
static const DECLARE_TLV_DB_SCALE(cod3034x_ctvol_spk_pga_tlv, -600, 100, 0);
 ...
 */
static const unsigned int cod3034x_dvol_adc_tlv[] = {
	TLV_DB_RANGE_HEAD(4),
 ...
 */
static const unsigned int cod3034x_dvol_dac_tlv[] = {
	TLV_DB_RANGE_HEAD(4),
...
static const struct snd_kcontrol_new cod3034x_snd_controls[] = {
	...
	SOC_SINGLE_TLV("Headphone Volume", COD3034X_57_AVC4,
			AVC_CTVOL_HP_SHIFT,
	...
	SOC_SINGLE_TLV("ADC Left Gain", COD3034X_47_AVOLL1,
			AD_DA_DVOL_SHIFT,
			AD_DVOL_MAXNUM, 1, cod3034x_dvol_adc_tlv),

	SOC_SINGLE_TLV("ADC Right Gain", COD3034X_48_AVOLR1,
			AD_DA_DVOL_SHIFT,
			AD_DVOL_MAXNUM, 1, cod3034x_dvol_adc_tlv),

	SOC_SINGLE_TLV("ADC Center Gain", COD3034X_4C_AVOLL2,
			AD_DA_DVOL_SHIFT,
			AD_DVOL_MAXNUM, 1, cod3034x_dvol_adc_tlv),

	SOC_DOUBLE_R_TLV("DAC Gain", COD3034X_51_DVOLL, COD3034X_52_DVOLR,
			AD_DA_DVOL_SHIFT,
			DA_DVOL_MAXNUM, 1, cod3034x_dvol_dac_tlv),
...
static int cod3034x_capture_init_manual_mode(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, COD3034X_19_CTRL_REF,
		CTMF_VMID_MASK, (CTMF_VMID_5K_OM << CTMF_VMID_SHIFT));
...
static int cod3034x_capture_init(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, COD3034X_4B_ADC2,
			ADC2_PDB_ADCDIG2_MASK, ADC2_PDB_ADCDIG2_MASK);

	cod3034x_capture_init_manual_mode(codec);
...
int cod3034x_mic_bias_ev(struct snd_soc_codec *codec, int mic_bias, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_update_bits(codec, COD3034X_10_PD_REF, mask, 0x0);
		if (mic_bias == COD3034X_MICBIAS2)
			snd_soc_update_bits(codec, COD3034X_19_CTRL_REF,
					CTRM_MCB2_MASK, 0x0);
		break;
...
static int cod3034_power_on_mic1(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, COD3034X_10_PD_REF,
			PDB_IGEN_AD_MASK|PDB_MCB1_MASK|PDB_MCB_LDO_CODEC_MASK,
			PDB_IGEN_AD_MASK|PDB_MCB1_MASK|PDB_MCB_LDO_CODEC_MASK);
	...
	/* Reset Micbias1 on */
	snd_soc_update_bits(codec, COD3034X_13_PD_AD3,
			RESETB_BST1_MASK, 0x0);

	/* Mixer path selection */
	snd_soc_update_bits(codec, COD3034X_26_MIX_ADC,
			EN_MIX_MIC1C_MASK, EN_MIX_MIC1C_MASK);

	msleep(140);

	/* EN_BST_DIODE, RESETB_BST1 OFF */
	snd_soc_update_bits(codec, COD3034X_13_PD_AD3, EN_BST_DIODE_MASK, 0x0);
	snd_soc_update_bits(codec, COD3034X_13_PD_AD3,
	...
	snd_soc_update_bits(codec, COD3034X_19_CTRL_REF,
			CTMF_VMID_MASK, (CTMF_VMID_50K_OM << CTMF_VMID_SHIFT));
...
static int cod3034_power_off_mic1(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, COD3034X_13_PD_AD3, RESETB_DSMC_MASK, 0x0);

	/* DSM OFF */
	snd_soc_update_bits(codec, COD3034X_11_PD_AD1,
			PDB_DSML_MASK|PDB_DSMR_MASK, 0x0);
	snd_soc_update_bits(codec, COD3034X_13_PD_AD3, PDB_DSMC_MASK, 0x0);

	/* MIXER OFF */
	snd_soc_update_bits(codec, COD3034X_11_PD_AD1,
			PDB_MIXR_MASK | PDB_MIXL_MASK, 0x0);
	snd_soc_update_bits(codec, COD3034X_13_PD_AD3, PDB_MIXC_MASK, 0x0);
...
static int cod3034_power_on_mic2(struct snd_soc_codec *codec)
{
	...
	/* format switch for 2mic enable */
	snd_soc_update_bits(codec, COD3034X_41_IF1_FORMAT1,
			LRCLK_POL_MASK, LRCLK_POL_MASK);
	snd_soc_update_bits(codec, COD3034X_44_IF1_FORMAT4,
			SEL_ADC1_MASK, ADC1_OUT);
	snd_soc_update_bits(codec, COD3034X_44_IF1_FORMAT4,
			SEL_ADC0_MASK, ADC2_OUT);

	/* MIC 2 enable SFR Start */
	snd_soc_update_bits(codec, COD3034X_10_PD_REF,
	...
	/* Reset Micbias2 on */
	snd_soc_update_bits(codec, COD3034X_13_PD_AD3,
			RESETB_BST2_MASK, 0x0);

	/* Mixer path selection */
	snd_soc_update_bits(codec, COD3034X_26_MIX_ADC,
			EN_MIX_MIC2C_MASK, EN_MIX_MIC2C_MASK);
...
static int cod3034_power_off_mic2(struct snd_soc_codec *codec)
{
	...
	/* 2 mic solution disable */
	snd_soc_update_bits(codec, COD3034X_41_IF1_FORMAT1,
			LRCLK_POL_MASK, 0x00);
	snd_soc_update_bits(codec, COD3034X_44_IF1_FORMAT4,
			SEL_ADC1_MASK, ADC2_OUT);
	snd_soc_update_bits(codec, COD3034X_44_IF1_FORMAT4,
			SEL_ADC0_MASK, ADC1_OUT);

	snd_soc_update_bits(codec, COD3034X_13_PD_AD3,
			EN_BST_DIODE_SHIFT, 0x00);
	/* 2 mic solution disable */
	...
	/* MIX Path OFF */
	snd_soc_update_bits(codec, COD3034X_26_MIX_ADC, EN_MIX_MIC2C_MASK, 0x0);

	/* DSM OFF */
	snd_soc_update_bits(codec, COD3034X_11_PD_AD1,
			PDB_DSMR_MASK | PDB_DSMR_MASK, 0x0);
	snd_soc_update_bits(codec, COD3034X_13_PD_AD3, PDB_DSMC_MASK, 0x0);
	...
	/* IGEN_AD OFF */
	snd_soc_update_bits(codec, COD3034X_10_PD_REF,
			PDB_IGEN_AD_MASK|PDB_MCB1_MASK|PDB_MCB_LDO_CODEC_MASK, 0x0);
...
static int cod3034_power_on_mic3(struct snd_soc_codec *codec)
{
	...
	/* ADC Current reference ON */
	snd_soc_update_bits(codec, COD3034X_10_PD_REF,
			PDB_IGEN_AD_MASK|PDB_MCB2_CODEC_MASK|PDB_MCB_LDO_CODEC_MASK,
			PDB_IGEN_AD_MASK|PDB_MCB2_CODEC_MASK|PDB_MCB_LDO_CODEC_MASK);
	...
	/* Reset Micbias3 on */
	snd_soc_update_bits(codec, COD3034X_13_PD_AD3, RESETB_BST3_MASK, 0x0);

	/* Mixer path selection */
	snd_soc_update_bits(codec, COD3034X_26_MIX_ADC,
			EN_MIX_MIC2C_MASK, EN_MIX_MIC3C_MASK);
...
static int cod3034_power_off_mic3(struct snd_soc_codec *codec)
{
	...
	/* MIX Path OFF */
	snd_soc_update_bits(codec, COD3034X_26_MIX_ADC, EN_MIX_MIC3C_MASK, 0x0);
	...
	snd_soc_update_bits(codec, COD3034X_10_PD_REF,
			PDB_IGEN_AD_MASK|PDB_MCB2_CODEC_MASK|PDB_MCB_LDO_CODEC_MASK, 0x0);
...
static void cod3034x_update_playback_otp(struct snd_soc_codec *codec)
{
	...
	if (hp_on && !spk_on && !ep_on) {
		...
		snd_soc_write(codec, COD3034X_D7_CTRL_EP, offset_ep);
		dev_dbg(codec->dev, "EP offset : 0x%x\n", offset_ep);
	} else if (hp_on && spk_on && !ep_on) {
...
static int cod3034x_hp_playback_init(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec, COD3034X_54_AVC1, 0x43);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		*/
		snd_soc_write(codec, COD3034X_54_AVC1, 0x00);
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		if (hp_on || ep_on) {
			...
			snd_soc_update_bits(codec, COD3034X_18_PWAUTO_DA,
					PW_AUTO_DA_MASK | APW_SPK_MASK, 0x00);
		}
		cod3034x_usleep(200);

		snd_soc_update_bits(codec, COD3034X_38_MIX_DA2,
				EN_SPK_MIX_DCTL_MASK | EN_SPK_MIX_DCTR_MASK, 0x00);
		...
		snd_soc_update_bits(codec, COD3034X_54_AVC1,
				AVC_BYPS_MASK, 0x00);
		*/
		snd_soc_write(codec, COD3034X_54_AVC1, 0x07);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		 ...
		 *		AVC_MU_EN_MASK, 0x0);
		 */
		snd_soc_write(codec, COD3034X_54_AVC1, 0x05);
		cod3034x_usleep(500);

		/* 0xDC <-- 0x5F: |0x07, CTRL_EPS */
		snd_soc_update_bits(codec, COD3034X_DC_CTRL_EPS,
				CTMI_EP_D_MASK, CTMI_EP_P_D_7_UA);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* bypass enable. */
		snd_soc_write(codec, COD3034X_54_AVC1, 0x0F);
		...
		msleep(136);
		/* no naming and register information for the 0x78 */
		snd_soc_write(codec, 0x78, 0x02);
...
static void cod3034x_jack_det_work(struct work_struct *work)
{
	...
	if (jackdet->jack_det && jackdet->mic_det) {
		/* 4 Pole Jack-in */
		snd_soc_write(codec, COD3034X_92_JACK_CTR, 0x30);
		/* button threshold */
		snd_soc_write(codec, COD3034X_88_CTR_IMP3, 0x10);
		dev_dbg(cod3034x->dev, "%s 4 Pole Jack-In\n", __func__);
	} else if (jackdet->jack_det) {
		/* 3 Pole Jack-in */
		snd_soc_write(codec, COD3034X_92_JACK_CTR, 0x20);
		/* button threshold */
		snd_soc_write(codec, COD3034X_88_CTR_IMP3, 0x10);
		dev_dbg(cod3034x->dev, "%s 3 Pole Jack-In\n", __func__);
	} else {
		/* Jack-out */
		snd_soc_write(codec, COD3034X_92_JACK_CTR, 0x00);
		dev_dbg(cod3034x->dev, "%s JACK OUT\n", __func__);
	}
...
static void cod3034x_codec_initialize(void *context)
{
	...
	/* Jack LDET Threshold */
	snd_soc_write(codec, COD3034X_83_CTR_VTH1, 0x32);
...
static void cod3034x_regmap_sync(struct device *dev)
{
	...
	regcache_cache_bypass(cod3034x->regmap, true);
	...
	/* Update HW */
	for (i = 0; i < COD3034X_REGCACHE_SYNC_END_REG ; i++)
		if (cod3034x_writeable_register(dev, i) &&
				(!cod3034x_volatile_register(dev, i)))
			snd_soc_write(cod3034x->codec, i, reg[i]);

	regcache_cache_bypass(cod3034x->regmap, false);
}
...
static int cod3034x_codec_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec, COD3034X_05_IRQ1M, 0xf2);
	snd_soc_write(codec, COD3034X_06_IRQ2M, 0xf2);
...
static int cod3034x_enable(struct device *dev)
{
	...
	regcache_cache_only(cod3034x->regmap, false);
...
static int cod3034x_disable(struct device *dev)
{
	...
	regcache_cache_only(cod3034x->regmap, true);
