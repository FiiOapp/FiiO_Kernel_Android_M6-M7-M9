...
static DEVICE_ATTR(usb_sel, 0664,
		s2mu004_muic_show_usb_sel, s2mu004_muic_set_usb_sel);
...
static int com_to_uart2(struct s2mu004_muic_data *muic_data)
{
	enum s2mu004_reg_manual_sw_value reg_val;
	int ret = 0;
...
static int switch_to_ap_uart(struct s2mu004_muic_data *muic_data)
{
	...
	val = s2mu004_i2c_read_byte(muic_data->i2c, 0xD4);
	val &= ~(0x80);
	ret = s2mu004_i2c_write_byte(muic_data->i2c, 0xD4, val);
...
static void s2mu004_muic_handle_detach(struct s2mu004_muic_data *muic_data)
{
	...
	bool noti = true;
		...
		muic_data->is_afc_muic_prepare = false;
#endif
	muic_data->rescan_status = RESCAN_NONE;
	ret = com_to_open(muic_data);
...
static void s2mu004_muic_detect_dev(struct s2mu004_muic_data *muic_data)
{
	...
	case DEV_TYPE1_USB:
		...
		if (muic_data->rescan_status != RESCAN_DONE) {
			muic_data->rescan_status = RESCAN_IDLE;
...
#ifdef CONFIG_MUIC_S2MU004_SUPPORT_DESKDOCK
		intr = MUIC_INTR_ATTACH;
...
#if 0
	ret = s2mu004_i2c_read_byte(i2c, S2MU004_MUIC_REG_DEVID);
	if (ret == 0x18)
		muic_data->rev_id = 2;
	else
		muic_data->rev_id = 0;
...
static void s2mu004_dcd_rescan(struct s2mu004_muic_data *muic_data)
{
	...
	pr_info("%s re-connect : 0x%X\n", __func__, reg_val);
	if (ret < 0)
		pr_err("[muic] %s err write MANSW(0x%x)\n",
			__func__, reg_val);
	muic_data->rescan_status = RESCAN_DONE;
