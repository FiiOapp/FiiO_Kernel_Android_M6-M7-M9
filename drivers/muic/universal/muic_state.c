...
void muic_detect_dev(muic_data_t *pmuic, int irq)
{
	...
	if (pmuic->vps_table == VPS_TYPE_SCATTERED) {
		...
		pr_info("%s:%s ST[0x%02x 0x%02x 0x%02x] BCCT[0x%02x 0x%02x] CT[0x%02x 0x%02x 0x%02x 0x%02x] HVCT[0x%02x 0x%02x]\n",
			...
			pr_info("%s:%s uid:%02x uidgnd:%02x adc:%02x vbvolt:%02x chgtyp:%02x chgdetrun:%02x\n",
			MUIC_DEV_NAME, __func__, pmuic->vps.t.uid, pmuic->vps.t.uidgnd,
			pmuic->vps.t.adc, pmuic->vps.t.vbvolt, pmuic->vps.t.chgtyp, pmuic->vps.t.chgdetrun);
#endif
	...
	if (val1 && CONTROL2_CPEN_MASK) {
		val2 = val1 && (u8)~CONTROL2_CPEN_MASK;
		ret = muic_i2c_write_byte(i2c, MAX77865_MUIC_REG_CONTROL2, val2);
	}
