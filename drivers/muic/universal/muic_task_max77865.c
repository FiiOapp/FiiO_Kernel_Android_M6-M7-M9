...
static irqreturn_t max77865_muic_irq_handler(muic_data_t *pmuic, int irq)
{
	...
	int irq_main, irq_bc, irq_fc, irq_gp, irq_ovp, irq_dcdtmr;
	int ret;
#if defined(CONFIG_MUIC_HV_MAX77865)
	...
	ret = muic_i2c_write_byte(i2c, MAX77865_REG_INT_MAIN, INT_MAIN_CLEAR);
	ret = muic_i2c_write_byte(i2c, MAX77865_REG_INT_BC, INT_BC_CLEAR);
	ret = muic_i2c_write_byte(i2c, MAX77865_REG_INT_FC, INT_FC_CLEAR);
	ret = muic_i2c_write_byte(i2c, MAX77865_REG_INT_GP, INT_GP_CLEAR);
...
static void muic_free_irqs(muic_data_t *pmuic)
{
	...
	/* free MUIC IRQ */
	FREE_IRQ(pmuic->irqs.irq_val, pmuic, "muic-intr");
}
