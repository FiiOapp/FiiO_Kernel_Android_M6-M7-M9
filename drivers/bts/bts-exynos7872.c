...
static void __iomem *base_drex_pf;
static void __iomem *base_sysreg_is;
...
static struct bts_info exynos_bts[] = {
	...
	[BTS_IDX_MFCMSCL] = {
		...
		.table[BS_CAMERA_DEFAULT].stat.wmo = 0x6,
	},
	[BTS_IDX_ABOX] = {
		.name = "abox",
	...
	[BTS_IDX_G3D] = {
		...
		.table[BS_DEFAULT].stat.priority = 0x4,
		.table[BS_CAMERA_DEFAULT].stat.priority = 0x4,
		.table[BS_CAMERA_DEFAULT].stat.rmo = 0x3,
		.table[BS_CAMERA_DEFAULT].stat.wmo = 0x3,
		.table[BS_DAOJIAN].stat.priority = 0x4,
		.table[BS_DAOJIAN].stat.rmo = 0x8,
		.table[BS_DAOJIAN].stat.wmo = 0x8,
	},
	[BTS_IDX_GNSS] = {
		.name = "gnss",
...
static void bts_add_scen(enum bts_scen_type scen)
{
	...
	if (scen == BS_CAMERA_DEFAULT) {
		__raw_writel(0x00200001, base_sysreg_is + XIU_D_IS_M0);
		__raw_writel(0x00280001, base_sysreg_is + XIU_D_IS_S3);
	}
}
...
static void bts_drex_init(void __iomem *base)
{
	...
	__raw_writel(0x00000005, base + CUTOFF_CONTROL);
	__raw_writel(0x00010001, base + BRB_CUTOFF_CONFIG0);
}

static void bts_drex_pf_init(void __iomem *base)
{
	__raw_writel(0x00000005, base + PORT_TOKEN_CONTROL);
	__raw_writel(0x00010001, base + PORT_TOKEN_THRESHOLD0);
}
					...
					size_t count, loff_t *ppos)
{
	...
	if (ip < 0 || ip > nr_ip || scen < 0 || scen > nr_scen || rw < 0 ||
			mo < 0) {
		pr_info("invalid variable\n");
		return len;
	}
					...
					size_t count, loff_t *ppos)
{
	...
	if (ip < 0 || ip > nr_ip || scen < 0 || scen > nr_scen || prio < 0 ||
			prio > 0xf) {
		pr_info("invalid variable\n");
		return len;
	}
...
static ssize_t set_mo_store(struct class *dev,
				struct class_attribute *attr, const char *buf, size_t size)
{
	...
	bts_update_scen(scen, on);

	return size;
}

static CLASS_ATTR(set_mo, 0644, NULL, set_mo_store);
...
static int __init exynos_bts_init(void)
{
	...
	if (!base_drex) {
		pr_err("failed to map base_drex physical address\n");
		ret = -ENOMEM;
		goto err0;
	}
	base_sysreg_is = ioremap(EXYNOS7872_PA_SYSREG_IS, SZ_4K);
	if (!base_sysreg_is) {
		pr_err("failed to map base_sysreg_is physical address\n");
		ret = -ENOMEM;
		goto err1;
	}
	base_drex_pf = ioremap(EXYNOS7872_PA_DREX_PF, SZ_4K);
	if (!base_drex_pf) {
		pr_err("failed to map base_drex_pf physical address\n");
		ret = -ENOMEM;
		goto err2;
	}
	bts_drex_init(base_drex);
	...
	bts_debugfs();
	bts_class = class_create(THIS_MODULE, "bts_set");
	if (IS_ERR(bts_class))
		pr_err("failed to create bts_class\n");

	ret = class_create_file(bts_class, &class_attr_set_mo);
	if (ret)
		pr_err("failed to create attr_set_mo\n");

	pr_info("BTS: driver is initialized\n");

	return 0;
err2:
	iounmap(base_sysreg_is);
err1:
	iounmap(base_drex);
err0:
	for (bts = exynos_bts;
