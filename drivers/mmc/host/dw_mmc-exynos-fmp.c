...
static struct bio *is_get_bio(struct mmc_data *data, bool cmdq_enabled)
{
	...
	if (cmdq_enabled) {
		...
		if (!cmdq_req || !virt_addr_valid(cmdq_req))
			return NULL;

		mrq = &cmdq_req->mrq;
		if (!mrq || !virt_addr_valid(mrq) ||
				!mrq->req || !virt_addr_valid(mrq->req) ||
				!mrq->req->bio || !virt_addr_valid(mrq->req->bio))
			return NULL;
		...
		if (!brq || !virt_addr_valid(brq))
			return NULL;

		mq_rq = container_of(brq, struct mmc_queue_req, brq);
		if (!mq_rq || !virt_addr_valid(mq_rq) ||
				!mq_rq->req || !virt_addr_valid(mq_rq->req) ||
				!mq_rq->req->bio || !virt_addr_valid(mq_rq->req->bio))
			return NULL;
		bio = mq_rq->req->bio;
	}
