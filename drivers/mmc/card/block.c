...
reset:
	...
	spin_lock_irqsave(&mq->eh_lock, flags);
	list_for_each_entry_safe(mrq_t, mrq_n, &mq->eh_mrq, eh_entry) {
		list_del(&mrq_t->eh_entry);
		BUG_ON(!mrq_t->req);
		pr_err("[CQ] %s:----- REQUEUE: tag %d\n",
				mmc_hostname(mrq_t->host), mrq_t->req->tag);
		exynos_ss_printk("[CQ] %s:----- REQUEUE: tag %d\n",
				mmc_hostname(mrq_t->host), mrq_t->req->tag);
	...
	pr_err("\n\n=============== CQ RECOVERY END ======================\n\n");
	exynos_ss_printk("\n\n=============== CQ RECOVERY END ======================\n\n");
	ctx_info->recovery_cnt++;
	if (ctx_info->recovery_cnt > 5)
		panic("recovery over!");
