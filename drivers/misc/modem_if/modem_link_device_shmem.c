...
static void max_qos_timer_callback(unsigned long data)
{
		...
		struct circ_status *circ)
{
	if (circ->size > SHM_RX_MAX_QOS_TRIGGER_BYTES) {
		...
		if (timer_pending(&shmd->max_qos_timer)) {
			/* Expand timer value */
			mod_timer(&shmd->max_qos_timer, get_jiffies_64() +
				msecs_to_jiffies(SHM_RX_MIN_QOS_HOLD_MS));
		} else {
			/* Set QOS ON */
			schedule_work(&shmd->pm_qos_work_max);
			mod_timer(&shmd->max_qos_timer, get_jiffies_64() +
				msecs_to_jiffies(SHM_RX_MIN_QOS_HOLD_MS));
		}
	}
...
static void shmem_qos_work_max(struct work_struct *work)
{
...
struct link_device *shmem_create_link_device(struct platform_device *pdev)
{
	...
	init_timer(&shmd->max_qos_timer);
	shmd->max_qos_timer.function = max_qos_timer_callback;
	shmd->max_qos_timer.data = (unsigned long)shmd;
