...
#endif
#include <soc/samsung/acpm_helpers.h>

#if !defined(CONFIG_SOC_EXYNOS7872) && !defined(CONFIG_SOC_EXYNOS7570)
#error Target processor CONFIG_SOC_EXYNOS7570 or CONFIG_SOC_EXYNOS7872 not selected
#endif
...
#if defined(CONFIG_SOC_EXYNOS7872) && defined(CONFIG_ACPM_DVFS)
			/* Immediately prior to reset release, set up ACPM
			 * to ensure BUCK2 gets the right voltage
			 */
			exynos_acpm_set_flag(MASTER_ID_WLBT, FLAG_LOCK);
#endif
			ret = platform_mif_pmu_reset_release(interface);
#if defined(CONFIG_SOC_EXYNOS7872) && defined(CONFIG_ACPM_DVFS)
			/* After reset release, set up ACPM
			 * to release BUCK2 locking.
			 */
			exynos_acpm_set_flag(MASTER_ID_WLBT, FLAG_UNLOCK);
#endif
		} else {
			/* Put back into reset */
