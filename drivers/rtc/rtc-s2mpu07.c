				 ...
				 enum S2M_RTC_OP op)
{
	...
	ret = s2mpu07_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, data);
	if (ret < 0)
...
static int s2m_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	ret = s2mpu07_bulk_read(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	mutex_lock(&info->lock);
	ret = s2mpu07_bulk_write(info->i2c, S2MP_RTC_REG_SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
...
static int s2m_rtc_check_rtc_time(struct s2m_rtc_info *info)
{
	...
	if (abs(rtc_time - sys_time.tv_sec) > 2) {
		...
		ret = s2mpu07_bulk_write(info->i2c, S2MP_RTC_REG_SEC,
...
static int s2m_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mpu07_bulk_read(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
	...
	ret = s2mpu07_read_reg(info->pmic_i2c, reg, &val); /* i2c for PM */
	if (ret < 0) {
...
static int s2m_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (info->alarm_check) {
		...
		ret = s2mpu07_bulk_write(info->i2c, S2MP_RTC_REG_A0SEC, NR_RTC_CNT_REGS,
				data);
		if (ret < 0) {
...
static inline int s2m_rtc_set_update_reg(struct s2m_rtc_info *info, enum S2M_RTC_OP op)
{
	...
	ret = s2mpu07_read_reg(info->i2c, S2MP_RTC_REG_UPDATE, &data);
	if (ret < 0)
		return ret;
...
static int s2m_rtc_stop_alarm_boot(struct s2m_rtc_info *info)
{
	...
	ret = s2mpu07_bulk_read(info->i2c, S2MP_RTC_REG_A1SEC, NR_RTC_CNT_REGS, data);
	if (ret < 0)
		return ret;

	s2m_data_to_tm(data, &tm);

	dev_info(info->dev, "%s: %d/%d/%d %d:%d:%d(%d)\n", __func__,
	...
	ret = s2mpu07_bulk_write(info->i2c, S2MP_RTC_REG_A1SEC, NR_RTC_CNT_REGS, data);
	if (ret < 0)
		return ret;
...
static int s2m_rtc_read_boot_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	if (ret < 0)
		goto out;
	ret = s2mpu07_bulk_read(info->i2c, S2MP_RTC_REG_A1SEC, NR_RTC_CNT_REGS,
			data);
	if (ret < 0) {
		dev_err(info->dev, "%s:%d fail to read alarm reg(%d)\n",
			__func__, __LINE__, ret);
	...
	ret = s2mpu07_read_reg(info->pmic_i2c, reg, &val);
	if (ret < 0) {
		dev_err(info->dev, "%s:%d fail to read STATUS2 reg(%d)\n",
			__func__, __LINE__, ret);
		goto out;
...
static int s2m_rtc_set_boot_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = s2mpu07_read_reg(info->i2c, S2MP_RTC_REG_UPDATE, &info->update_reg);
	if (ret < 0) {
	...
	ret = s2mpu07_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, (char)info->update_reg);
						...
						struct s2mpu07_platform_data *pdata)
{
	...
	if (pdata->osc_bias_up >= 0) {
		ret = s2mpu07_update_reg(info->i2c, S2MP_RTC_REG_CAPSEL,
	...
	if (pdata->cap_sel >= 0) {
		s2mpu07_update_reg(info->i2c, S2MP_RTC_REG_CAPSEL,
	...
	if (pdata->osc_xin >= 0) {
		s2mpu07_update_reg(info->i2c, S2MP_RTC_REG_OSCCTRL,
...
static bool s2m_is_jigonb_low(struct s2m_rtc_info *info)
{
	...
	ret = s2mpu07_read_reg(info->i2c, reg, &val);
	if (ret < 0) {
						...
						struct s2mpu07_platform_data *pdata)
{
	...
	ret = s2mpu07_write_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, val);
	if (ret < 0) {
					...
					struct s2mpu07_platform_data *pdata)
{
	...
	ret = s2mpu07_update_reg(info->i2c, S2MP_RTC_REG_WTSR_SMPL, 0,
				...
				struct s2mpu07_platform_data *pdata)
{
	...
	ret = s2mpu07_bulk_read(info->i2c, S2MP_RTC_REG_A1SEC, NR_RTC_CNT_REGS, data_alm1);
	if (ret < 0)
		return ret;
	...
	ret = s2mpu07_read_reg(info->i2c, S2MP_RTC_REG_UPDATE, &update_val);
	if (ret < 0) {
	...
	ret = s2mpu07_write_reg(info->i2c, S2MP_RTC_REG_UPDATE, info->update_reg);
	if (ret < 0) {
		dev_err(info->dev, "%s: fail to write update reg(%d)\n",
			__func__, ret);
		return ret;
	...
	ret = s2mpu07_read_reg(info->i2c, S2MP_RTC_REG_CTRL, &ctrl_val);
	if (ret < 0) {
	...
	ret = s2mpu07_read_reg(info->i2c, S2MP_RTC_REG_CAPSEL, &capsel_val);
	if (ret < 0) {
	...
	data = MODEL24_MASK;
	ret = s2mpu07_write_reg(info->i2c, S2MP_RTC_REG_CTRL, data);
	if (ret < 0) {
	...
	capsel_val |= 0xf0;
	ret = s2mpu07_write_reg(info->i2c, S2MP_RTC_REG_CAPSEL, capsel_val);
	if (ret < 0) {
...
static int s2m_rtc_probe(struct platform_device *pdev)
{
	...
	struct s2mpu07_platform_data *pdata = dev_get_platdata(iodev->dev);
	...
	switch (info->iodev->device_type) {
	case S2MPU07X:
		info->irq = irq_base + S2MPU07_PMIC_IRQ_RTCA0_INT2;
#if defined(CONFIG_RTC_BOOT_ALARM)
		info->alarm_boot_irq = irq_base + S2MPU07_PMIC_IRQ_RTCA1_INT2;
#endif
		break;
	default:
		/* If this happens the core funtion has a problem */
