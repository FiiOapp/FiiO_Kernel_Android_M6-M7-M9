				    ...
				    unsigned int *val)
{
	...
	pr_debug("%s : request prop = %d , attached_state = %d, power_role_dual = %d\n",
		...
		u8 int0, u8 int1, u8 int2, u8 int3, u8 int4)
{
	...
	dev_err(dev, "%s : 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", __func__, int_mask[0], int_mask[1], int_mask[2], int_mask[3], int_mask[4]);
	if (ret < 0)
...
int s2mu004_set_normal_mode(struct s2mu004_usbpd_data *pdic_data)
{
	...
	dev_info(dev, "%s : 0x%x, 0x%x, 0x%x\n", __func__, S2MU004_REG_INT_STATUS2_MSG_SRC_CAP, ENABLED_INT_3, ENABLED_INT_4_PASS);
		...
		unsigned long action, void *data)
{
	...
	if ((action == MUIC_PDIC_NOTIFY_CMD_ATTACH) &&
		...
		pr_info("%s, ATTACHED_DEV_WATER_MUIC\n", __func__);
		...
		attached_dev == ATTACHED_DEV_WATER_MUIC) {
		pr_info("%s, DETACH ATTACHED_DEV_WATER_MUIC\n", __func__);
		...
		if (pdic_data->is_otg_reboost) {
			...
			pr_info("%s, CHECK_OCP, Can't afford it\n", __func__);
#if 0
				...
				const struct i2c_device_id *id)
{
	...
	dev_err(dev, "%s: rid = %d , init_port = %d\n", __func__, rid, init_port);
