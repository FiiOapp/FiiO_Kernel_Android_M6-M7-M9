...
retry:
	...
	if (list_empty(&m21dev->active_contexts[0]) &&
		list_empty(&m21dev->active_contexts[1])) {
		/* no context to process */
		spin_unlock_irqrestore(&m21dev->lock_ctx, flags);
		return;
	}

	idx = (m21dev->resource_sharing) ? 1 : 0;
	m21dev->resource_sharing = !m21dev->resource_sharing;

	/*
	 * Since selected job didn't run this time,
	 * we change the order one more so that it can be processed first.
	 */
	if (list_empty(&m21dev->active_contexts[idx])) {
		idx ^= 1;
		m21dev->resource_sharing = !m21dev->resource_sharing;
	}

	ctx = list_first_entry(&m21dev->active_contexts[idx],
...
static void m2m1shot2_schedule_context(struct m2m1shot2_context *ctx)
{
	 ...
	 * critical job(real-time job) is moved on 1st active contexts,
	 * non-critical job is moved on 0st active contexts.
	 */
	idx = !!(ctx->flags & M2M1SHOT2_FLAG_CRITICAL_JOB) ? 1 : 0;

	list_move_tail(&ctx->node, &ctx->m21dev->active_contexts[idx]);
					...
					unsigned long attr)
{
	...
	INIT_LIST_HEAD(&m21dev->active_contexts[0]);
	INIT_LIST_HEAD(&m21dev->active_contexts[1]);
