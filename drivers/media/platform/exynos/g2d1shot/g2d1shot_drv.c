...
static int m2m1shot2_g2d_init_context(struct m2m1shot2_context *ctx)
{
	...
	init_completion(&g2d_ctx->complete);
	complete_all(&g2d_ctx->complete); /* prevent to wait for completion */
...
static bool g2d1shot_still_need_perf(struct m2m1shot2_context *ctx)
{
	...
	list_for_each_entry(pctx, &m21dev->active_contexts[0], node) {
		if (pctx->priority == ctx->priority)
			goto need_perf;
	}

	list_for_each_entry(pctx, &m21dev->active_contexts[1], node) {
		if (pctx->priority == ctx->priority)
			goto need_perf;
	}
...
static void warmup_stage_for_nv12(struct m2m1shot2_context *ctx)
{
	struct g2d1shot_ctx *g2d_ctx = ctx->priv;
	struct g2d1shot_dev *g2d_dev = g2d_ctx->g2d_dev;
	struct m2m1shot2_context_format *ctx_fmt;
	struct g2d1shot_fmt *fmt;
	dma_addr_t addr, addr_cb;
	unsigned long flags;
	u32 w, h, crop_w;
	int i;
	...
	for (i = 0; i < ctx->num_sources; i++) {
		...
		fmt = ctx_fmt->priv;
		w = ctx_fmt->fmt.width;
		h = ctx_fmt->fmt.height;
		crop_w = ctx_fmt->fmt.crop.width;
		...
		reinit_completion(&g2d_ctx->complete);

		spin_lock_irqsave(&g2d_dev->state_lock, flags);
		g2d_dev->state |= G2D_STATE_WARMUP;
		spin_unlock_irqrestore(&g2d_dev->state_lock, flags);

		g2d_hw_init(g2d_dev);

		addr = m2m1shot2_src_dma_addr(ctx, i, 0);
		if (fmt->pixelformat == V4L2_PIX_FMT_NV12N)
			...
			addr_cb = m2m1shot2_src_dma_addr(ctx, i, 1);
		else /* contiguous format */
			addr_cb = addr + w * h;

		/* set address on cbcr region */
		__raw_writel(addr_cb, g2d_dev->reg + G2D_DST_BASE_ADDR_REG);
		__raw_writel(addr_cb, g2d_dev->reg + G2D_LAYER0_BASE_ADDR_REG);

		/* set width and height */
		__raw_writel(w / 2, g2d_dev->reg + G2D_LAYER0_IMAGE_WIDTH_REG);
		__raw_writel(h / 2, g2d_dev->reg + G2D_LAYER0_IMAGE_HEIGHT_REG);
		__raw_writel(w / 2, g2d_dev->reg + G2D_DST_IMAGE_WIDTH_REG);
		__raw_writel(h / 2,	g2d_dev->reg + G2D_DST_IMAGE_HEIGHT_REG);

		/* set dst */
		__raw_writel(0, g2d_dev->reg + G2D_DST_LEFT_REG);
		__raw_writel(0, g2d_dev->reg + G2D_DST_TOP_REG);
		__raw_writel(w / 2, g2d_dev->reg + G2D_DST_RIGHT_REG);
		__raw_writel(h / 2, g2d_dev->reg + G2D_DST_BOTTOM_REG);

		/* set source rect */
		__raw_writel(crop_w / 2 - 1, g2d_dev->reg + G2D_LAYER0_LEFT_REG);
		__raw_writel(0, g2d_dev->reg + G2D_LAYER0_TOP_REG);
		__raw_writel(crop_w / 2, g2d_dev->reg + G2D_LAYER0_RIGHT_REG);
		__raw_writel(h / 2, g2d_dev->reg + G2D_LAYER0_BOTTOM_REG);

		/* set dest clip */
		__raw_writel(crop_w / 2, g2d_dev->reg + G2D_LAYER0_DST_LEFT_REG);
		__raw_writel(0, g2d_dev->reg + G2D_LAYER0_DST_TOP_REG);
		__raw_writel(crop_w / 2 + 1, g2d_dev->reg + G2D_LAYER0_DST_RIGHT_REG);
		__raw_writel(h / 2, g2d_dev->reg + G2D_LAYER0_DST_BOTTOM_REG);

		/* set format */
		__raw_writel(G2D_DATAFORMAT_RGB4444 | G2D_SWIZZLING_ARGB,
			g2d_dev->reg + G2D_LAYER0_COLOR_MODE_REG);
		__raw_writel(G2D_DATAFORMAT_RGB4444 | G2D_SWIZZLING_ARGB,
			g2d_dev->reg + G2D_DST_COLOR_MODE_REG);

		/* set external memory */
		__raw_writel(0, g2d_dev->reg + G2D_LAYER0_SELECT_REG);

		/* set stride */
		__raw_writel(w, g2d_dev->reg + G2D_LAYER0_STRIDE_REG);
		__raw_writel(w, g2d_dev->reg + G2D_DST_STRIDE_REG);

		/* set valid flag */
		__raw_writel(G2D_LAYER_VALID,
			g2d_dev->reg + G2D_LAYER0_COMMAND_REG);

		/* set update layer flag */
		__raw_writel(1, g2d_dev->reg + G2D_LAYER_UPDATE_REG);

		g2d_dev->t_start = sched_clock();
		/* run H/W */
		g2d_hw_start(g2d_dev);

		wait_for_completion(&g2d_ctx->complete);
...
static int m2m1shot2_g2d_device_run(struct m2m1shot2_context *ctx)
{
	...
	/* execute w/a for nv12 cropping */
	warmup_stage_for_nv12(ctx);
...
static irqreturn_t exynos_g2d_irq_handler(int irq, void *priv)
{
	...
	warmup = g2d_dev->state & G2D_STATE_WARMUP;
	spin_unlock_irqrestore(&g2d_dev->state_lock, flags);
	...
	if (!suspending) {
		schedule_work(&g2d_dev->irq_bh_work);
	} else {
...
static void g2d1shot_irq_bh_work(struct work_struct *work)
{
	struct g2d1shot_dev *g2d_dev =
		container_of(work, struct g2d1shot_dev, irq_bh_work);
...
static int exynos_g2d_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&g2d_dev->irq_bh_work, g2d1shot_irq_bh_work);
