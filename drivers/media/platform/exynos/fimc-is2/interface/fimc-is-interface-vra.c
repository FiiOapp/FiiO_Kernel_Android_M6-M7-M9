	...
	struct fimc_is_frame *frame)
{
	...
	if (enable_mcsc_otfout_size_change_flag) {
		/* Convert face size */
		for (i = 0; i < CAMERA2_MAX_FACES; i++) {
			if (frame->shot->dm.stats.faceScores[i] == 0)
				continue;
			frame->shot->dm.stats.faceRectangles[i][0] =
					CONVRES(frame->shot->dm.stats.faceRectangles[i][0],
					fd_width, saved_vra_ori_input_width);
			frame->shot->dm.stats.faceRectangles[i][1] =
					CONVRES(frame->shot->dm.stats.faceRectangles[i][1],
					fd_height, saved_vra_ori_input_height);
			frame->shot->dm.stats.faceRectangles[i][2] =
					CONVRES(frame->shot->dm.stats.faceRectangles[i][2],
					fd_width, saved_vra_ori_input_width);
			frame->shot->dm.stats.faceRectangles[i][3] =
					CONVRES(frame->shot->dm.stats.faceRectangles[i][3],
					fd_height, saved_vra_ori_input_height);
		}
	}
#endif
