...
bool check_dma_done(struct fimc_is_hw_ip *hw_ip, u32 instance_id, u32 fcount)
{
	...
	if (frame == NULL) {
		mserr_hw("[F:%d]check_dma_done: frame(null)!!", instance_id, hw_ip, fcount);
		return ret;
	}

	/*
	 * The expected_fcount in which num_buffers is considered.
	 * It would be matched with frame->fcount which has "HW_WAIT_DOEN" state.
	 *  ex. frame->fcount : 1, num_buffer : 4, fcount : 4 => expected_fcount : 1
	 */
	expected_fcount = fcount - (frame->num_buffers - 1);

	msdbg_hw("check_dma [F:%d], frame[F:%d], num_buffers:%d, expected_fcount:%d\n",
			instance_id, hw_ip, fcount, frame->fcount,
			frame->num_buffers, expected_fcount);
	...
	if (((frame->num_buffers > 1) && (expected_fcount != frame->fcount))
		...
		framemgr_x_barrier(framemgr, 0);
		if (list_frame == NULL) {
			mswarn_hw("[F:%d,exp:%d] invalid frame(%d,idx:%d)",
				instance_id, hw_ip, fcount, expected_fcount,
	...
	if (test_bit(output_id0, &frame->out_flag)) {
		msdbg_hw("[F:%d]output_id[0x%x],wq_id[0x%x]\n",
	...
	u32 instance_id, void *data)
{
	...
	case LIB_EVENT_FRAME_END:
		...
		if (frame_done)
			...
			msdbg_hw("dma done interupt separate\n", instance_id, hw_ip);
