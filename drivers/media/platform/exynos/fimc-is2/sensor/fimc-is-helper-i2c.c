	...
	u16 addr, u16 val)
{
	...
	if (ret < 0) {
		pr_err("i2c treansfer fail(%d)", ret);
#ifdef CONFIG_SOC_EXYNOS7872
		is_regs = ioremap_nocache(0x148F0020, SZ_4);
		rear_reset_status = readl(is_regs);
		pr_info("%s: rear_reset_status(0x%x)\n", __func__, rear_reset_status);
		iounmap(is_regs);
#endif
		goto p_err;
	}
