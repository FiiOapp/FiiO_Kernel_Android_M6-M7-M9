	...
	struct is_region *region, u32 lindex, u32 hindex, ulong hw_map)
{
	...
	msdbg_hw("set_param hw_map[0x%lx]\n", instance, hw_ip, hw_map);
	...
	u32 instance, ulong hw_map, u32 output_id, enum ShotErrorType done_type)
{
	...
	if ((output_id != FIMC_IS_HW_CORE_END)
		 ...
		 */
		msdbg_hw("%s: skip to get_meta [F:%d][B:0x%lx][C:0x%lx][O:0x%lx]\n",
...
void fimc_is_set_hw_count(struct fimc_is_hardware *hardware, struct fimc_is_group *head,
	u32 instance, u32 fcount, ulong hw_map)
{
	...
	while (child && (child->device_type == FIMC_IS_DEVICE_ISCHAIN)) {
		...
		for (hw_index = hw_maxnum - 1; hw_index >= 0; hw_index--) {
			...
			if (test_bit_variables(hw_ip->id, &hw_map)) {
				...
				msdbg_hw("[F:%d]count clear, fs(%d->%d), fe(%d->%d), dma(%d->%d)\n",
	...
	struct fimc_is_group *group, struct fimc_is_frame *frame, ulong hw_map)
{
	...
	fimc_is_set_hw_count(hardware, head, instance, frame->fcount, hw_map);
...
void fimc_is_hardware_frame_start(struct fimc_is_hw_ip *hw_ip, u32 instance)
{
	...
	if (atomic_read(&hw_ip->status.otf_start)
		...
		/* error handling */
		sinfo_hw("frame_start_isr (F%d, fs(%d))\n", hw_ip,
			frame->fcount, atomic_read(&hw_ip->count.fs));
	...
	u32 group_id)
{
	...
	for (hw_index = 0; hw_index < hw_maxnum; hw_index++) {
		...
		hardware = hw_ip->hardware;
		msinfo_hw("process_start [G:0x%x], framemgr[%s]->framemgr[%s]\n",
			instance, hw_ip, GROUP_ID(group_id),
	...
	struct fimc_is_hw_ip *hw_ip, u32 instance)
{
	...
	msinfo_hw("frame_list (R%d, P%d, C%d)\n", instance, hw_ip,
		framemgr->queued_count[FS_HW_REQUEST],
		framemgr->queued_count[FS_HW_CONFIGURE],
		framemgr->queued_count[FS_HW_WAIT_DONE]);
	...
	while (--retry && framemgr->queued_count[FS_HW_WAIT_DONE]) {
		...
		msinfo_hw("complete_list [F:%d][T:%d][O:0x%lx][C:0x%lx](C%d)", instance, hw_ip,
			frame->fcount, frame->type, frame->out_flag, frame->core_flag,
			framemgr->queued_count[FS_HW_WAIT_DONE]);
		ret = fimc_is_hardware_frame_ndone(hw_ip, frame, instance, IS_SHOT_UNPROCESSED);
		if (ret) {
	...
	while (--retry && framemgr->queued_count[FS_HW_CONFIGURE]) {
		...
		msinfo_hw("process_list [F:%d][T:%d][O:0x%lx][C:0x%lx](P%d)", instance, hw_ip,
			frame->fcount, frame->type, frame->out_flag, frame->core_flag,
			framemgr->queued_count[FS_HW_CONFIGURE]);
		...
		mswarn_hw(" %d pro waiting...", instance, hw_ip,
			framemgr->queued_count[FS_HW_CONFIGURE]);
		usleep_range(1000, 1000);
	}
	...
	while (--retry && framemgr->queued_count[FS_HW_REQUEST]) {
		...
		msinfo_hw("request_list [F:%d](R%d)", instance, hw_ip,
			frame->fcount, framemgr->queued_count[FS_HW_REQUEST]);
		...
		mswarn_hw(" %d req waiting...", instance, hw_ip,
			framemgr->queued_count[FS_HW_REQUEST]);
		usleep_range(1000, 1000);
	}

	msinfo_hw("late_list (R%d, P%d, C%d)\n", instance, hw_ip,
	...
	for (list_index = FS_HW_REQUEST; list_index < FS_HW_INVALID; list_index++) {
		...
		while (--retry && framemgr_late->queued_count[list_index]) {
			...
			msinfo_hw("late_list[%d] [F:%d][T:%d][O:0x%lx][C:0x%lx](%d)", instance, hw_ip,
			...
			mswarn_hw(" %d late waiting...", instance, hw_ip,
				framemgr_late->queued_count[list_index]);
			usleep_range(1000, 1000);
		}
	}
	...
	struct fimc_is_group *group, u32 instance, bool rep_flag, u32 module_id)
{
	...
	if (refcount == 1) {
		...
		msdbg_hw("%s: [G:0x%x], framemgr[ID:0x%x]\n",
...
int fimc_is_hardware_close(struct fimc_is_hardware *hardware,u32 hw_id, u32 instance)
{
	...
	if (refcount == 0) {
		...
		msdbg_hw("%s: [G:0x%x], framemgr[ID:0x%x]->framemgr[ID:0x%x]\n",
			instance, hw_ip, __func__, GROUP_ID(group_id),
	...
	u32 output_id, enum ShotErrorType done_type)
{
	...
	if (hw_fcount > frame->fcount) {
		msdbg_hw("LATE %s END [F:%d][0x%x][C:0x%lx][O:0x%lx][END:%d]\n",
			frame->instance, hw_ip, type, frame->fcount,
			output_id, frame->core_flag, frame->out_flag,
			hw_fcount);

		msinfo_hw("%d: force_done for LATE %s END [F:%d][HWF:%d]\n",
			frame->instance, hw_ip, __LINE__, type, frame->fcount, hw_fcount);
	...
	int wq_id, u32 output_id, enum ShotErrorType done_type)
{
	...
	case IS_SHOT_SUCCESS:
		if (frame == NULL) {
			...
			swarn_hw("frame NOT null!!(%d)", hw_ip, done_type);
		}
		break;
...
free_frame:
	...
	if (frame->type == SHOT_TYPE_INTERNAL) {
		msdbg_hw("INTERNAL_SHOT_DONE [F:%d][G:0x%x]\n",
			frame->instance, hw_ip, frame->fcount, GROUP_ID(head->id));
		...
		msdbg_hw("SHOT_TYPE_MULTI [F:%d][G:0x%x]\n",
			frame->instance, hw_ip, frame->fcount, GROUP_ID(head->id));
#endif
	} else {
		msdbg_hw("SHOT_DONE [F:%d][G:0x%x]\n",
			frame->instance, hw_ip, frame->fcount, GROUP_ID(head->id));
		atomic_set(&hw_ip->hardware->log_count, 0);
	}
exit:
	...
	struct __setfile_table_entry *setfile_table_entry)
{
	...
	for (idx = 0; idx < setfile->using_count; idx++) {
		...
		msdbg_hw("setfile[%d] addr: 0x%lx, size: %x\n",
			instance, hw_ip, idx,
			setfile->table[idx].addr,
	...
	for (ip = 0; ip < header.num_ips; ip++) {
		...
		while (hw_slot != HW_SLOT_MAX) {
			...
			if (!test_bit(hw_ip->id, &hardware->hw_map[instance])) {
				msdbg_hw("skip parsing at not mapped hw_ip", instance, hw_ip);
				continue;
			}
