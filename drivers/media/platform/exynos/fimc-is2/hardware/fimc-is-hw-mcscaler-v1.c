	...
	ulong hw_map)
{
	...
	msdbg_hw("[F:%d]shot [T:%d] cmd[%d] addr[0x%x]\n",
	...
	struct param_scaler_input_crop *in_crop, u32 instance) /* poly_phase */
{
	...
	msdbg_hw("poly_phase_setting cmd(O:%d,D:%d)\n",
	...
	if ((post_en && ((poly_dst_width << MCSC_POST_WA_SHIFT) % out_width))
		...
		msdbg_hw("poly_phase: shift(%d), ratio(%d,%d), "
		...
		msdbg_hw("poly_phase: size(%dx%d) after  calculation\n",
	...
	u32 instance)
{
	...
	msdbg_hw("post_chain_setting cmd(O:%d,D:%d)\n",
		instance, hw_ip, otf_output->cmd, dma_output->cmd);
	...
	struct param_scaler_flip *flip, u32 instance)
{
	...
	msdbg_hw("flip_setting flip(%d),cmd(D:%d)\n",
	...
	struct param_otf_output *otf_output, u32 instance)
{
	...
	msdbg_hw("otf_output_setting cmd(O:%d,D:%d)\n",
		instance, hw_ip, otf_output->cmd, dma_output->cmd);
	...
	struct param_dma_output *dma_output, u32 instance)
{
	...
	msdbg_hw("dma_output_setting cmd(O:%d,D:%d)\n",
		instance, hw_ip, otf_output->cmd, dma_output->cmd);
	...
	if ((scaled_width != out_width) || (scaled_height != out_height)) {
		msdbg_hw("Invalid scaled size (%d/%d)(%d/%d)\n",
