...
static int fimc_is_hw_vra_ch0_handle_interrupt(u32 id, void *context)
{
	...
	sdbg_hw("isr[ch0]: id(%d), status(0x%x), mask(0x%x), status(0x%x)\n",
...
static int fimc_is_hw_vra_ch1_handle_interrupt(u32 id, void *context)
{
	...
	status = (intr_mask) & intr_status;
	msdbg_hw("isr[ch1]: id(%d), status(0x%x), mask(0x%x), status(0x%x)\n",
	...
	struct fimc_is_group *group)
{
	...
	get_lib_vra_func((void *)&hw_vra->lib_vra.itf_func);
#endif
	msdbg_hw("library interface was initialized\n", instance, hw_ip);
...
new_frame:
	...
	/* At VRA enabled first, sysreg ops do for S/W workarond */
	if (test_bit(FIMC_IS_GROUP_OTF_INPUT, &head->state)
		&& test_bit(VRA_LIB_FWALGS_ABORT, &lib_vra->state)
		&& atomic_read(&head->scount) > 0)
		sysreg_ops = true;
	...
	msdbg_hw("[F:%d]lib_vra_new_frame\n", instance, hw_ip, frame->fcount);
	...
	/* At VRA enabled first, sysreg ops do for S/W workarond */
	if (sysreg_ops) {
		fimc_is_hw_cfg_sysreg(hw_ip->hardware->base_addr_mcuctl, DEV_HW_VRA);
		sysreg_ops = false;
	}
