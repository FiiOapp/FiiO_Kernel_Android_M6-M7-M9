	...
	struct fimc_is_group *group)
{
	...
	msdbg_hw("open: [G:0x%x], framemgr[%s]", instance, hw_ip,
...
skip_addr:
	...
	for (i = MCSC_OUTPUT0; i < cap->max_output; i++) {
		...
		msdbg_hw("[F:%d]wdma_cfg [T:%d][addr%d: %x]\n", frame->instance, hw_ip,
		...
		if (param->output[i].dma_cmd != DMA_OUTPUT_COMMAND_DISABLE
			...
			msdbg_hw("[OUT:%d]dma_out enabled\n", frame->instance, hw_ip, i);
			...
			if (wdma_enable && (cap->enable_shared_output == false || !test_bit(i, &hw_mcsc_out_configured))) {
				...
				msdbg_hw("[OUT:%d]shot: dma_out disabled\n",
						frame->instance, hw_ip, i);
			}
			spin_unlock_irqrestore(&shared_output_slock, flag);
			msdbg_hw("[OUT:%d]mcsc_wdma_cfg:wmda_enable(%d)[F:%d][T:%d][cmd:%d][addr:0x%x]\n",
...
config:
	...
	msdbg_hw("shot: hw_mcsc_out_configured[0x%lx]\n", instance, hw_ip,
	...
	struct mcs_param *param, u32 lindex, u32 hindex, u32 instance)
{
	...
	if (hw_mcsc->instance != instance) {
		control_cmd = true;
		msdbg_hw("update_param: hw_ip->instance(%d), control_cmd(%d)\n",
	...
	u32 instance, ulong hw_map)
{
	...
	hw_mcsc->setfile = (struct hw_api_scaler_setfile *)setfile->table[setfile_index].addr;
	...
	ulong hw_map)
{
	...
	if (!test_bit(HW_TUNESET, &hw_ip->state)) {
		msdbg_hw("setfile already deleted", instance, hw_ip);
		return 0;
	}
	...
	int done_type)
{
	...
	msdbgs_hw(2, "frame done[F:%d][O:0x%lx]\n", instance, hw_ip,
		done_frame->fcount, done_frame->out_flag);
	...
	u32 instance)
{
	...
	msdbg_hw("otf_input_setting cmd(%d)\n", instance, hw_ip, input->otf_cmd);
	width  = input->width;
	height = input->height;
	...
	u32 instance)
{
	...
	msdbg_hw("dma_input_setting cmd(%d)\n", instance, hw_ip, input->dma_cmd);
	...
	struct param_mcs_output *output, u32 output_id, u32 instance)
{
	...
	msdbg_hw("[OUT:%d]poly_phase_setting cmd(O:%d,D:%d)\n",
	...
	if ((post_en && ((poly_dst_width << MCSC_POST_WA_SHIFT) % out_width))
		...
		msdbg_hw("[OUT:%d]poly_phase: shift(%d), ratio(%d,%d), "
		...
		msdbg_hw("[OUT:%d]poly_phase: size(%dx%d) after  calculation\n",
	...
	/* HACK(only 7872) : save poly dst size for use post chain */
	output->poly_dst_width = poly_dst_width;
	output->poly_dst_height = poly_dst_height;
	...
	struct param_mcs_output *output, u32 output_id, u32 instance)
{
	...
	msdbg_hw("[OUT:%d]post_chain_setting cmd(O:%d,D:%d)\n",
		instance, hw_ip, output_id, output->otf_cmd, output->dma_cmd);
	...
	/* HACK(only 7872) : get poly dst size for use post chain */
	img_width = output->poly_dst_width;
	img_height = output->poly_dst_height;
	...
	u32 output_id, u32 instance)
{
	...
	msdbg_hw("[OUT:%d]flip_setting flip(%d),cmd(O:%d,D:%d)\n",
	...
	u32 output_id, u32 instance)
{
	...
	msdbg_hw("[OUT:%d]otf_output_setting cmd(O:%d,D:%d)\n",
		instance, hw_ip, output_id, output->otf_cmd, output->dma_cmd);
	...
	u32 output_id, u32 instance)
{
	...
	msdbg_hw("[OUT:%d]dma_output_setting cmd(O:%d,D:%d)\n",
		instance, hw_ip, output_id, output->otf_cmd, output->dma_cmd);
...
/* HACK */
#ifndef CONFIG_SOC_EXYNOS7872
	if ((scaled_width != out_width) || (scaled_height != out_height)) {
		msdbg_hw("Invalid output[%d] scaled size (%d/%d)(%d/%d)\n",
	...
	u32 hwfc_output_ids, u32 instance)
{
	...
	msdbg_hw("hwfc_mode_setting output[0x%08X]\n", instance, hw_ip, hwfc_output_ids);
	...
	u32 output_id, u32 instance)
{
	...
	msdbg_hw("hwfc_config_setting mode(%dx%d, %d, %d)\n", instance, hw_ip,
	...
	u32 output_id, u32 instance)
{
	...
	if (test_bit(HW_TUNESET, &hw_ip->state)) {
		...
		msdbg_hw("set YUV range(%d) by setfile parameter\n",
			instance, hw_ip, yuv_range);
		msdbg_hw("[OUT:%d]output_yuv_setting: yuv_range(%d), cmd(O:%d,D:%d)\n",
		...
		msdbg_hw("YUV range set default settings\n", instance, hw_ip);
	}
#else
	...
	bool start_flag)
{
	...
	if (noise_index >= max_ref_ni) {
		dbg_hw("current NI (%d) > Max ref NI(%d), set bypass mode", noise_index, max_ref_ni);
		*tdnr_mode = TDNR_MODE_BYPASS;
		goto exit;
	} else if (noise_index <= min_ref_ni) {
		dbg_hw("current NI (%d) < Min ref NI(%d), use first NI value", noise_index, min_ref_ni);
		bottom_ni_index = 0;
		top_ni_index = 0;
	} else {
