	...
	struct pinctrl *pinctrl, struct exynos_sensor_pin *pin_ctrls)
{
	...
	case PIN_OUTPUT:
		/*
		 * Maestor7872 board is wrong schematic
		 * because audio and sensor reset is same gpio pin
		 * and happen to problem when sensor reset control after audio power domain off
		 * so add a check status to audio power
		 */
		if (cal_pd_status(0xB1380000) == 0) {
			err("power off to aud so don't working camera at maestro7872");
			break;
		}
