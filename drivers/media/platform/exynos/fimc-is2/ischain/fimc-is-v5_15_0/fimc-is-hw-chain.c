...
static struct fimc_is_reg sysreg_is_regs[SYSREG_IS_REG_CNT] = {
	...
	{0x0820, "IS_PIXELASYNC_SW_RESET_2"},
...
static struct fimc_is_field sysreg_is_fields[SYSREG_IS_REG_FIELD_CNT] = {
	...
	{"PIXELASYNCM_MCSC_VRA",	8,	1,	RW,	0x1},	/* IS_PIXELASYNC_SW_RESET_2 */
	{"PIXELASYNCS_MCSC_VRA",	0,	1,	RW,	0x1},	/* IS_PIXELASYNC_SW_RESET_2 */
	{"MIPI_PHY_MUX_SEL",		24,	1,	RW,	0x0},	/* IS_MIPI_DPHY_CTRL */
};
...
int fimc_is_hw_group_open(void *group_data)
{
	...
	case GROUP_ID_MCS0:
		...
		leader->constraints_height = 3032;
		break;
	default:
...
void fimc_is_hw_cfg_sysreg(void __iomem *sysreg_addr, int hw_id)
{
	BUG_ON(!sysreg_addr);

	switch (hw_id) {
	case DEV_HW_VRA:
		/* S/W workaround for VRA input(only for Katmai)
		 * - as VRA disable -> enable during streaming,
		 *   enabled VRA first frame interrupt is not occured(H/W bug)
		 *   So, between VRA global enable and frame start interrupt,
		 *   MCSC-VRA pixelasync SW-reset must do for prevent Hang
		 */
		fimc_is_hw_set_reg(sysreg_addr, &sysreg_is_regs[SYSREG_IS_R_PIXELASYNC_SW_RESET_2], 0x0);
		fimc_is_hw_set_reg(sysreg_addr, &sysreg_is_regs[SYSREG_IS_R_PIXELASYNC_SW_RESET_2], 0x101);
		break;
	default:
		err_itfc("%s: hw_id(%d) not exists", __func__, hw_id);
	}
}
