...
static struct pm_qos_request perf_int;
static struct pm_qos_request perf_mif;
static struct pm_qos_request perf_cpu[2];
#define CHIPID_SIZE	(16)
#ifdef CONFIG_EXYNOS_CHIPID
static char boot_serialno[CHIPID_SIZE + 1];
int androidboot_serialno = 0;
#endif
...
struct gadget_info {
	...
	unsigned int max_mif;
	unsigned int max_int;
	unsigned int max_cl0;
	unsigned int max_cl1;
	int parse_fail;
#endif
};
...
#ifdef CONFIG_ECT
static int exynos_devfreq_parse_ect(struct gadget_info *dev, char *dvfs_domain_name)
{
	int i, counter = 0;
	void *dvfs_block;
	...
	if (!strcmp(dvfs_domain_name, "dvfs_mif")) {
		dev->mif_table.num_of_table = dvfs_domain->num_of_level;
		for (i = dvfs_domain->num_of_level - 1; i >= 0; i--) {
			dev->mif_table.freq[i] = dvfs_domain->list_level[counter++].level;
			pr_debug("MIF_LEV[%d] : %u\n", i + 1, dev->mif_table.freq[i]);
			if (dev->max_mif < dev->mif_table.freq[i])
				dev->max_mif = dev->mif_table.freq[i];
		}
	} else if (!strcmp(dvfs_domain_name, "dvfs_cpucl0")) {
		dev->cl0_table.num_of_table = dvfs_domain->num_of_level;
		for (i = dvfs_domain->num_of_level - 1; i >= 0; i--) {
			dev->cl0_table.freq[i] = dvfs_domain->list_level[counter++].level;
			pr_debug("CL0_LEV[%d] : %u\n", i + 1, dev->cl0_table.freq[i]);
			if (dev->max_cl0 < dev->cl0_table.freq[i])
				dev->max_cl0 = dev->cl0_table.freq[i];
		}
	} else if (!strcmp(dvfs_domain_name, "dvfs_cpucl1")) {
		dev->cl1_table.num_of_table = dvfs_domain->num_of_level;
		for (i = dvfs_domain->num_of_level - 1; i >= 0; i--) {
			dev->cl1_table.freq[i] = dvfs_domain->list_level[counter++].level;
			pr_debug("CL1_LEV[%d] : %u\n", i + 1, dev->cl1_table.freq[i]);
			if (dev->max_cl1 < dev->cl1_table.freq[i])
				dev->max_cl1 = dev->cl1_table.freq[i];
		}
	} else if (!strcmp(dvfs_domain_name, "dvfs_int")) {
		dev->int_table.num_of_table = dvfs_domain->num_of_level;
		for (i = dvfs_domain->num_of_level - 1; i >= 0; i--) {
			dev->int_table.freq[i] = dvfs_domain->list_level[counter++].level;
			pr_debug("INT_LEV[%d] : %u\n", i + 1, dev->int_table.freq[i]);
			if (dev->max_int < dev->int_table.freq[i])
				dev->max_int = dev->int_table.freq[i];
		}
	}
...
#ifdef CONFIG_EXYNOS_CHIPID
static int __init boot_serialno_setup(char *str)
{
    if(!str)
        return -EINVAL;
    memset(boot_serialno, 0, CHIPID_SIZE + 1);
    memcpy(boot_serialno, str, min((size_t)CHIPID_SIZE, strlen(str)));
    androidboot_serialno = 1;
    pr_info("boot_serialno_setup:boot_serialno : %s\n",
                boot_serialno);
    return 1;
}
__setup("androidboot.serialno=", boot_serialno_setup);
...
//	snprintf(str, CHIPID_SIZE + 1, "%016lx", (long)exynos_soc_info.unique_id);
      if(androidboot_serialno) {
            memcpy(str, boot_serialno, CHIPID_SIZE + 1);
      }
      else {
            snprintf(str, CHIPID_SIZE + 1, "%016lx", (long)exynos_soc_info.unique_id);
      }
					...
					const char *buff, size_t size)
{
	...
	while (b) {
		...
		if (!strcmp(name, "rndis")) {
			if (!dev->parse_fail) {
				pr_info("Set Max freq for rndis performance\n");
				pr_info("MIF level : %u \n", dev->max_mif);
				pr_info("INT level : %u \n", dev->max_int);
				pr_info("CL0 level : %u \n", dev->max_cl1);
				pr_info("CL1 level : %u \n", dev->max_cl0);
				pm_qos_update_request(&perf_int, dev->max_int);
				pm_qos_update_request(&perf_mif, dev->max_mif);
				pm_qos_update_request(&perf_cpu[0], dev->max_cl0);
				pm_qos_update_request(&perf_cpu[1], dev->max_cl1);
			} else {
				pr_info("Can't get MAX freq, Skip freq setting for RNDIS\n");
			}
		}
#endif
	}
			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled) {
		...
		pm_qos_update_request(&perf_int, 0);
		pm_qos_update_request(&perf_mif, 0);
		pm_qos_update_request(&perf_cpu[0], 0);
		pm_qos_update_request(&perf_cpu[1], 0);
	} else {
		pr_err("%s: already %s\n", __func__,
		...
		const char *name)
{
	...
	pr_info("USB %s: Parsing devfreq and cpufreq table from ECT\n", __func__);
	ret = exynos_devfreq_parse_ect(gi, "dvfs_mif");
	if (ret < 0) {
		pr_err("Can't get MIF table\n");
		goto err1;
	}

	ret = exynos_devfreq_parse_ect(gi, "dvfs_cpucl0");
	if (ret < 0) {
		pr_err("Can't get CPU table\n");
		goto err1;
	}

	ret = exynos_devfreq_parse_ect(gi, "dvfs_cpucl1");
	if (ret < 0) {
		pr_err("Can't get CPU table\n");
		goto err1;
	}

	ret = exynos_devfreq_parse_ect(gi, "dvfs_int");
	if (ret < 0) {
		pr_err("Can't get INT table\n");
		goto err1;
	}
#endif
	return &gi->group;
...
#ifdef CONFIG_ECT
err1:
	pr_err("failed to parse ECT table, skip to get the ECT table\n");
	gi->parse_fail = 1;
...
static int __init gadget_cfs_init(void)
{
	...
	pm_qos_add_request(&perf_int, PM_QOS_DEVICE_THROUGHPUT, 0);
	pm_qos_add_request(&perf_mif, PM_QOS_BUS_THROUGHPUT, 0);
	pm_qos_add_request(&perf_cpu[0], PM_QOS_CLUSTER0_FREQ_MIN, 0);
	pm_qos_add_request(&perf_cpu[1], PM_QOS_CLUSTER1_FREQ_MIN, 0);
