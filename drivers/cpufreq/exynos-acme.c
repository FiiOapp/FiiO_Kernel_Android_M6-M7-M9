...
static bool last_domain(struct exynos_cpufreq_domain *domain)
{
	return domain == list_last_entry(&domains,
...
static int exynos_cpufreq_suspend(struct cpufreq_policy *policy)
{
	...
	pm_qos_update_request(&domain->min_qos_req, domain->resume_freq);
	pm_qos_update_request(&domain->max_qos_req, domain->resume_freq);

	/*
	 * To guarantee applying resume freq, update_freq() is called
	 * explicitly.
	 */
	update_freq(domain, domain->resume_freq);
                                ...
                                struct attribute *attr, char *buf)
{
	...
	list_for_each_entry_reverse(domain, &domains, list) {
		 ...
		 * big is not shown.
		 */
		if (last_domain(domain) && !get_hmp_boost())
			continue;
#endif
	 ...
	 */
	return snprintf(buf, 10, "%u\n",
			domain->min_freq >> (scale * SCALE_SIZE));
}
				...
				size_t count)
{
	...
	list_for_each_entry_reverse(domain, &domains, list) {
		...
		if (input < (domain->min_freq >> (scale * SCALE_SIZE))) {
			pm_qos_update_request(&domain->user_min_qos_req,
						domain->min_freq);
			continue;
		}
