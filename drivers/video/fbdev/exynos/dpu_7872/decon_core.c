...
#if defined(CONFIG_EXYNOS_DECON_DQE)
static void __dqe_dump(struct decon_device *decon)
{
	decon_info("\n=== DQE SFR DUMP ===\n");
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
			decon->res.regs + 0x2000, 0x324, false);

	decon_info("\n=== DQE SHADOW SFR DUMP ===\n");
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
			decon->res.regs + 0x9000, 0x324, false);
}
#endif
...
err:
	...
	dqe_restore_context();
	dqe_reg_start(decon->id, decon->lcd_info->xres, decon->lcd_info->yres);
#endif
	mutex_unlock(&decon->lock);
	return ret;
}
...
#if defined(CONFIG_EXYNOS_DECON_DQE)
	dqe_save_context(); /* dqe_save should be executed before decon_stop */
...
#if defined(CONFIG_EXYNOS_DECON_DQE)
	ret = decon_create_dqe_interface(decon);
	if (ret)
		goto err_pinctrl;
#endif
