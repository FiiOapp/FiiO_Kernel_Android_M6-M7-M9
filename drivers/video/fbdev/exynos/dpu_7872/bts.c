...
#define PPC			1UL	/* Katmai PPC : 1 */
#define LCD_REFRESH_RATE	63UL
#define MULTI_FACTOR 		(1UL << 10)
#define BTS_DMA_MAX			4

static int dpu_bts_log;
struct bts_decon_info bts_log_info[3];
...
void dpu_bts_calc_bw(struct decon_device *decon, struct decon_reg_data *regs)
{
	...
	if (dpu_bts_log)
		memcpy(&bts_log_info[decon->id], &bts_info, sizeof(struct bts_decon_info));
...
static void dpu_bts_log_info_output(struct decon_device *decon)
{
	int i;
	struct bts_decon_info *bts_info = &bts_log_info[decon->id];
	...
	for (i = 0; i < BTS_DPP_MAX; ++i) {
		...
		if (!bts_info->dpp[i].used)
			continue;
		if (dpu_bts_log >= 1)
			pr_info("[BTS] [%d] (%d) (%4d, %4d)\t(%4d, %4d, %4d, %4d)\n",
				bts_info->dpp[i].idma_type, bts_info->dpp[i].bpp,
				bts_info->dpp[i].src_w, bts_info->dpp[i].src_h,
				bts_info->dpp[i].dst.x1, bts_info->dpp[i].dst.y1,
				bts_info->dpp[i].dst.x2, bts_info->dpp[i].dst.y2);
	}
	if (dpu_bts_log >= 2)
		pr_info("[BTS] BW(KB/s): type%d bw %up %ur\n",
...
void dpu_bts_init(struct decon_device *decon)
{
	...
	if (!debugfs_create_u32("bts", 0644, decon->d.debug_root, &dpu_bts_log))
		pr_err("could't create debugfs dpu bts log\n");
}
