...
static void exynos_print_cp_status(void)
{
	u32 cp_ctrl;
...
int exynos_cp_reset(void)
{
	...
	pr_err("%s: call exynos_acpm_set_flag(MASTER_ID_CP, FLAG_LOCK) before reset CP\n", __func__);
	exynos_acpm_set_flag(MASTER_ID_CP, FLAG_LOCK);
#endif
...
exit:
#if defined(CONFIG_SOC_EXYNOS7872)
	pr_err("%s: call exynos_acpm_set_flag(MASTER_ID_CP, FLAG_UNLOCK) after reset CP\n", __func__);
	exynos_acpm_set_flag(MASTER_ID_CP, FLAG_UNLOCK);
#endif
...
#if defined(CONFIG_SOC_EXYNOS7872)
	pr_err("%s: call exynos_acpm_set_flag(MASTER_ID_CP, FLAG_LOCK) before release CP\n", __func__);
	exynos_acpm_set_flag(MASTER_ID_CP, FLAG_LOCK);
#endif
...
exit:
#if defined(CONFIG_SOC_EXYNOS7872)
	pr_err("%s: call exynos_acpm_set_flag(MASTER_ID_CP, FLAG_UNLOCK) after release CP\n", __func__);
	exynos_acpm_set_flag(MASTER_ID_CP, FLAG_UNLOCK);
#endif
...
int exynos_set_cp_power_onoff(enum cp_mode mode)
{
	...
	if (mode == CP_POWER_ON) {
#if defined(CONFIG_SOC_EXYNOS7872)
		pr_err("%s: call exynos_acpm_set_flag(MASTER_ID_CP, FLAG_LOCK) before CP on\n", __func__);
		exynos_acpm_set_flag(MASTER_ID_CP, FLAG_LOCK);
#endif
		if (!(cp_ctrl & CP_PWRON)) {
		...
		if (cp_ctrl == -1) {
#if defined(CONFIG_SOC_EXYNOS7872)
			pr_err("%s: call exynos_acpm_set_flag(MASTER_ID_CP, FLAG_UNLOCK) after CP on\n", __func__);
			exynos_acpm_set_flag(MASTER_ID_CP, FLAG_UNLOCK);
#endif
			return -1;
		}
