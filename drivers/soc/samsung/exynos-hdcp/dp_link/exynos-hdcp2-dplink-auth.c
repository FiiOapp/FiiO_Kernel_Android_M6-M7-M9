...
static void cal_rcvid_list_size(uint8_t *rxinfo, uint32_t *rcvid_size)
{
	struct rxinfo rx;

	rxinfo_convert_arr2st(rxinfo, &rx);
#if defined(HDCP_DEBUG)
	rxinfo_print_all_info(&rx);
#endif
...
static int do_recv_rcvid_list(struct hdcp_link_data *lk)
{
	...
	cal_rcvid_list_size(m_rcvid.rx_info, &rcvid_size);
...
static int check_pairing_ready(void)
{
	...
	while (i < 3) {
		...
		if (status & DP_RXSTATUS_PAIRING_AVAILABLE) {
			hdcp_info("pairing is ready. time(%dms), status(%x)\n", i, status);
			return 0;
		}
...
static int check_rcvidlist_ready(void)
{
	...
	mdelay(200);
	/* HDCP spec is 3 sec. but we give margin */
	while (i < 6) {
		hdcp_dplink_recv(HDCP22_MSG_RXSTATUS_R, &status, sizeof(uint8_t));
		if (status & DP_RXSTATUS_READY) {
			hdcp_info("receiver ID list is ready. time(%dms), status(%x)\n", (i * 500), status);
			return 0;
		}
...
int dplink_exchange_master_key(struct hdcp_link_data *lk)
{
	...
	do {
		...
		if (lk->stored_km == HDCP_WITHOUT_STORED_KM) {
			...
			/* give delay before read pairing info */
			mdelay(10);
