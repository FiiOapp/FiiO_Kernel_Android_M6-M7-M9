...
int exynos_acpm_set_flag(u32 master_id, bool is_lock)
{
	...
	config.cmd[0] = id;
	config.cmd[1] = ((1 << master_id) << 16) | (is_lock << master_id);
...
static int acpm_dvfs_probe(struct platform_device *pdev)
{
	...
	acpm_ipc_request_channel(acpm_noti_mif.dev,
...
{
	acpm_ipc_request_channel(acpm_dvfs.dev,
				 NULL,
				 &acpm_dvfs.ch_num,
				 &acpm_dvfs.size);
