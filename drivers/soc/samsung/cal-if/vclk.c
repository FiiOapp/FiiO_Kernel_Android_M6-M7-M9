...
unsigned long vclk_recalc_rate(unsigned int id)
{
	...
	if (IS_DFS_VCLK(vclk->id) ||
		...
		if (vclk->ops && vclk->ops->get_pll)
			vclk->vrate = vclk->ops->get_pll(id);
		else
...
static int vclk_get_dfs_info(struct vclk *vclk)
{
	...
	for (i = 0; i < vclk->num_rates; i++) {
		...
		params = kzalloc(sizeof(struct vclk_lut) * vclk->num_list,
				 GFP_KERNEL);
		if (!params) {
			ret = -EVCLKNOMEM;
			if (i == 0)
...
static int vclk_get_asv_info(struct vclk *vclk)
{
	...
	get_max_min_freq_lv(domain, asv_table_ver, &max_lv, &min_lv);
	if (max_lv >= 0)
	...
	table = &domain->table_list[asv_table_ver];
